...
export class LlamaCppProvider extends BaseProvider {
  ...
  constructor(config?: Partial<ProviderConfig>) {
    super()
    this.baseUrl = config?.base_url || 'http://localhost:8080/v1'
    this.apiKey = config?.api_key_encrypted || ''
    this.timeout = config?.timeout_seconds || 120
  }
  ...
  async testConnection(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      return {
        success: false,
        message: `无法连接到 llama.cpp (${this.baseUrl}): ${message}`,
      }
    }
  }
